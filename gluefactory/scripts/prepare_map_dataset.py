...
DEFAULTS = {
    ...
    # The database lives at data/Map1/Map1.db, i.e. inside this directory, and
    # the extracted images are written next to it. --clean removes only the
    # generated entries (see GENERATED_ENTRIES) so the database survives.
...
# Everything this script generates inside data_dir. --clean removes exactly
# these and nothing else, so a data_dir that also holds the source database
# (data/MAP1/Map1.db sits next to the extracted images) survives.
GENERATED_ENTRIES = (
    "images/rgb",
    "images/depth",
    "images/calib",
...
def safe_clean(data_dir: Path, db: Path) -> None:
    """Remove only this script's generated artifacts from data_dir.

    data_dir is an rmtree in the general case, and the deployed layout puts the
    source database *inside* data_dir (data/MAP1/Map1.db alongside the
    extracted images). The database is the one input that cannot be recreated
    here, so delete by known name instead of wiping the directory, and verify
    it is still there afterwards.
    """
    data_dir_r = data_dir.resolve()
    db_r = db.resolve()
    project_r = PROJECT_ROOT.resolve()

    if not data_dir_r.is_dir():
        logger.info(f"nothing to clean, {data_dir_r} does not exist")
        return
    if data_dir_r == project_r or data_dir_r in project_r.parents:
        raise SystemExit(
            f"refusing to --clean {data_dir_r}: it is the repository root or "
            f"an ancestor of it."
        )
    if data_dir_r in (Path(data_dir_r.anchor), Path.home()):
        raise SystemExit(f"refusing to --clean {data_dir_r}: too broad a path.")

    removed = 0.0
    for rel in GENERATED_ENTRIES:
        target = data_dir_r / rel
        if not target.exists():
            continue
        removed += _dir_size_mb(target)
        if target.is_dir():
            shutil.rmtree(target, ignore_errors=True)
        else:
            target.unlink(missing_ok=True)

    logger.info(f"cleaned {removed:.1f} MB of generated data from {data_dir_r}")

    # The whole point of the exercise: prove the database is still there.
    if db_r.exists():
        logger.info(f"source database intact: {db_r}")
    elif data_dir_r in db_r.parents or db_r.parent == data_dir_r:
        raise SystemExit(
            f"source database {db_r} is gone after --clean. It cannot be "
            f"regenerated by this script."
        )
