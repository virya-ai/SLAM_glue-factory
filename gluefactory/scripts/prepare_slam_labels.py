...
Usage:
    python -m gluefactory.scripts.prepare_slam_labels \\
...
def main():
    ...
    parser.add_argument("--num_vis", type=int, default=50,
                        help="Number of pseudo-label images to visualize "
                             "automatically after creation (0 = skip).")
    ...
    if args.num_vis and args.num_vis > 0:
        logger.info(
            f"Auto-visualizing {args.num_vis} pseudo-label images after creation..."
        )
        visualize(
            "labels",
        ...
        logger.info("Pseudo-label visualization complete.")
