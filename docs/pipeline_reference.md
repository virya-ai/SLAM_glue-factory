...
| `extract_features` | Batch-export keypoints + descriptors to HDF5 for HPatches/eth3d benchmarks |
| `extract_slam_features` | Export features at consensus keypoints (SLAM SuperGlue/LightGlue training) |
| `trace_model` | Convert a trained checkpoint to a standalone `.pt` for deployment |
| `prepare_slam_labels` | Generate SLAM-format pseudo-labels with pose-driven 3D warps |
| `prepare_slam_pairs` | Build SLAM pair list + ground-truth from pose file and image dir |
...
```bash
# SuperPoint
python3 -m gluefactory.scripts.trace_model \
    --config gluefactory/configs/superpoint_slam.yaml \
...
│       └── ...
└── exports/
    ├── slam_pairs.csv          ← generated by prepare_slam_pairs
